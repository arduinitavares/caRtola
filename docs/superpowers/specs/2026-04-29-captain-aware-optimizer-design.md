...
- default captain scoring to enabled;
...
No new data source, captain model, fixture data, or compatibility audit is required.
...
- add `captain_mode: "enabled" | "disabled"` with default `enabled`;
...
If existing output columns named `predicted_points` and `actual_points` remain, they must be aliases for the active scoring contract:
